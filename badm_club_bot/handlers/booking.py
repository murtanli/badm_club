...
from texts.session_text import session_text, session_text_busy
...
async def process_trainer_choice(callback: CallbackQuery):
	...
	if sport_list:
		...
		text = (
			"❌ Тренировок на неделю не найдены"
		)
...
async def get_training_info(callback: CallbackQuery):
	training_session = int(callback.data.split("-")[-1])
	data = await get_session_data(training_session, callback.from_user.id)
	not_available = False

	if data['bookings_count'] == data['max_participants']:
		session_text_lines = session_text_busy(
			data['type_name'],
			data['start_datetime'],
			data['end_datetime'],
			data['gym_name'],
		...
		await callback.message.edit_text(session_text_lines, reply_markup=inline_keyboard)
	else:
